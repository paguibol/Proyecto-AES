...
import os
import sys
from common import adb, connection, go_home, open_bbklogs, take_screenshot, get_cfg, write_time_to_Excel_2_columns, write_start_end_time_test_to_Excel, fill_excel_with_basic_info, get_number_SIM
...
def close_log(d):
    ...
    d(resourceId="android:id/edit").set_text("MMS_3G_Prepago")
    sleep(2)
...
def mms(phone_number, repetitions=5, interval=60):
    ...
    for i in range(repetitions):
        ...
        resultado =write_time_to_Excel_2_columns(i+1, current_time, col_a="K", col_b="L", start_row=36, NW="3G", total_reps=repetitions)    #Escribe en Excel, pasar en que columnas, fila y RAT empieza a escribir,   
        ...
        write_start_end_time_test_to_Excel(tiempo_inicio, tiempo_fin, col_c="E", col_d="F", start_row=21, NW="3G")  # Escribe en Excel el tiempo de la primera y última iteración del test, en las columnas C y D respectivamente, para la tecnología 3G. 
...
def main():
    ...
    fill_excel_with_basic_info(NW="3G", SIM_number=number_10_digits, Linea ="Prepago con saldo")  #Llena en el excel el modelo y la fecha, en Línea colocar: "Pospago", "Prepago sin saldo" o "Prepago con saldo" dependiendo del tipo de línea que se esté probando
    close_settings(d)
    close_log(d)
